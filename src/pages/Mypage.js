...
class Mypage extends React.Component {
  ...
  updatemyplant = () => {
    ...
    if (image && name) {
      ...
      axios
        .post(
          "http://plantingg.com/plant/upload",
          {
            image,
            name,
          }, //서버에서 받아오는 변수명 수정 필요
  ...
  componentDidUpdate() {
    //mypage 정보 업데이트시 이 코드 실행
    axios
      .get("http://plantingg.com/plant/get", {
      ...
      <div>
        ...
        <div>
          ...
          <span className="currentstatus">
            {this.props.userinfo.currentstatus}
          </span>
          <button className="btn-logout" onClick={this.setmode}>
            update userinfo
          </button>
        ...
        <div>
          {this.state.name.map(
            (name) => (
              <span className="plants" key={name}>
                {name}
              </span>
            ) //사진도 함께 하려면 어떻게 해야 할까 ?
          )}
        </div>
        <button className="btn-updateplany" onClick={this.updatemyplant}>
          update myplant
        </button>
